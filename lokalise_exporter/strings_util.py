# Taken from: https://github.com/chrisballinger/python-localizable/blob/master/localizable.py
# because including it as dependency gave runtime errors on Python 3.6
"""
...
def read_strings_file_as_dict(logger, file_path, underscorize_keys):
    strings = parse_strings(filename=file_path)

    properties = {}

    for string in strings:
        key = '"' + underscorize(string['key'], underscorize_keys) + '"'
        value = string['value'].strip().replace('"', '\\"')
